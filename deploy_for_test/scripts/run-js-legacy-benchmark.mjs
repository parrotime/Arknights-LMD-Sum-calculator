...
function filterItemsForLegacy(items, settings) {
  return items.filter((item) => {
    const type = String(item.type || "").toLowerCase();
    const isUpgradeAllowed = settings.allowUpgradeOnlyFor1 ? type !== "upgrade" : true;
    return (
      (settings.allow3Star || type !== "3_star") &&
      (settings.allow2Star || type !== "2_star") &&
      (settings.allowMaterial || type !== "material") &&
      (settings.allowStore20 || type !== "store_20") &&
      (settings.allowStore10 || type !== "store_10") &&
      (settings.allowStore70 || type !== "store_70") &&
      (settings.allowStore2000 || type !== "store_2000") &&
      (settings.allowStore5000 || type !== "store_5000") &&
      (settings.allowCE || type !== "ce") &&
      (settings.allowExt25 || type !== "ext_25") &&
      (settings.allowTrade || type !== "trade") &&
      (settings.allowUpgradeOnly0 || type !== "upgrade_only_0") &&
      (settings.allowUpgradeOnly1 || type !== "upgrade_only_1") &&
      (settings.allowUpgradeOnly2 || type !== "upgrade_only_2") &&
      (settings.allowOrundumsGreen || Number(item.id) !== 220) &&
      (settings.allowOrundumsDevice || Number(item.id) !== 221) &&
      isUpgradeAllowed
    );
  });
}
...
function buildLegacyBody(testCase, allItems) {
  const items = filterItemsForLegacy(allItems, testCase.settings);
  return {
...
function makeRecord({ testCase, run, response, itemCount }) {
...
function markdownReport(payload) {
  const lines = [
    "# JS Legacy Benchmark Report",
    "",
    `- backend: js-legacy`,
    `- url: ${payload.url}`,
    `- createdAt: ${payload.createdAt}`,
    `- caseFile: ${payload.caseFile}`,
    `- itemsFile: ${payload.itemsFile}`,
    `- runs: ${payload.runs}`,
...
const itemsFile = path.resolve(args["items-file"] || path.join(ROOT, "..", "data", "gameItems.json"));
...
const allItems = readJson(itemsFile);
...
for (const testCase of cases) {
  const body = buildLegacyBody(testCase, allItems);
  for (let run = 1; run <= runs; run += 1) {
    ...
    const record = makeRecord({ testCase, run, response, itemCount: body.items.length });
    records.push(record);
    ...
    console.log(`${mark} ${testCase.group}/${testCase.name} run=${run} status=${record.status} cache=${record.cache || "-"} client=${record.durationMsClient}ms server=${record.durationMsServer ?? "-"}ms paths=${record.pathCount}`);
  }
}
...
const jsonFile = path.join(reportDir, `js-legacy-benchmark-${labelPart}${stamp}.json`);
const jsonlFile = path.join(reportDir, `js-legacy-benchmark-${labelPart}${stamp}.jsonl`);
const mdFile = path.join(reportDir, `js-legacy-benchmark-${labelPart}${stamp}.md`);
