...
当前 `finalizeResult` 中的多样性处理成本较低，因为它只处理已经进入 target 的少量候选路径，通常最多十几条；真正的性能热点仍在搜索阶段、路径合并、normalize、key 生成和关卡组合 DP。

后续优化 fast/strong 模式时可以评估：

- `fast`：保留必要的去重、排序、劣质路径过滤，考虑跳过或简化 sig 多样性选择，以减少结果后处理分支并保持结果更直接。
- `strong`：保留完整多样性选择，优先保证 10 条结果的类型覆盖和展示质量。
- `hasRemovableZeroSumSubset` 建议默认保留，它解决的是可删除正负抵消子集这类劣质路径，不只是展示多样性问题。

是否拆分为模式特供逻辑，需要以后续 benchmark 和典型样例人工验收为准。
