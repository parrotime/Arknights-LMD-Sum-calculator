...
function trimDp(dp, target) {
  ...
  dp.clear();
  for (const [sum, paths] of sortedStates.slice(0, MAX_STATES)) {
    dp.set(sum, paths);
  }
  ...
  for (let step = 2; step <= MAX_STEPS; step++) {
    ...
    for (const [currentSum, paths] of currentStates) {
        //////////////////////////////////////////////////////////////////////////
      //if (Math.abs(currentSum - target) > Math.min(3000, Math.abs(target) * 2)) {
      if (Math.abs(currentSum - target) > 3000) {
        console.log(`剪枝: ${currentSum} 偏离 ${target} 过远`);
        continue;
      }
      for (const item of sortedItems) {
        const itemValue = item.item_value;
